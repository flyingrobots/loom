...
This DAG is the execution ordering for Milestone 6. It is phase-based so status can be derived from checkboxes.

```mermaid
flowchart TD
  %% Status is auto-updated by scripts/update_roadmap_dags.py
  P0[Phase 0: Freeze task model<br/>ids + state machine + receipts] --> P1[Phase 1: Kernel implementation<br/>submitIntent + lifecycle]
  P1 --> P2[Phase 2: API wiring<br/>resolvers + subscriptions]
  P2 --> P3[Phase 3: Tests<br/>task run + replay]
