...
This DAG is the execution ordering for Milestone 3. It is phase-based so status can be derived from checkboxes.

```mermaid
flowchart TD
  %% Status is auto-updated by scripts/update_roadmap_dags.py
  P0[Phase 0: Freeze conflict semantics<br/>conflicts + receipts + spec] --> P1[Phase 1: Kernel collapse<br/>SWS → System + events]
  P1 --> P2[Phase 2: API wiring<br/>collapseSws + error codes]
  P2 --> P3[Phase 3: Tests<br/>unit + HTTP replay]
